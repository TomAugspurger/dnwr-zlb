...
I adopt a finding from recent research on wage rigidities: idiosyncratic shocks are important for generating aggregate effects on output or employment.\footnote{
...
Instead, we focus on the consequences of wage downward rigidities.\footnote{
...
I use a a relatively simple model closely following \cite{daly_hobijn_2013} to examine consequences of wage rigidities.
...
While the specific details of wage rigidity varied across countries, a few common themes emerged:
...
surveys, firm-level employment records, panel datasets, and the CPS.
Of these, panel datasets such as the Panel Study of Income Dynamics (PSID) in the U.S. tend to give the smallests estimates of rigidity [citation needed].
...
Anything below the limiting case of $G_t(w (1 + \pi)) = 1$ ends up lowering output.  %TODO: Really super bad.
...
But the usual result (as in, say, \citep{daly_hobijn_2013}) is that this would offset but not overcome the loss of output due workers being stuck at too high a wage.
...
But instead of the output level increases instead of decreases as the degree of wage rigidity increases.
